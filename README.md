...
- **The app gets scored, not just tested** — and the lowest score becomes the next improvement target. Loop.

## The Agentic UI Bar (B1–B11)

Each dimension scored 0–2 per pass; lowest = next improvement target:
...
B8 is the meta-dimension: if a Haiku-class agent can't complete the core journeys from your app's profile alone, the friction list *is* your improvement spec.
...
A low Bar score isn't the end of the pass. Pick exactly one primary mode by the mechanism:

| The change needed | Primary mode |
|---|---|
| Existing UI should be removed, merged, deferred, compacted, or repaired in place | [`DECLUTTER.md`](DECLUTTER.md) — structural-subtractive |
| A capability, honest state, route, proposal affordance, or information architecture/layout is missing | [`REVAMP.md`](REVAMP.md) — structural-additive |
| Rendered structure, visible content, behavior, and a11y stay equivalent; only visual tokens change | [`PRETTIFY.md`](PRETTIFY.md) — presentation-only |

Finish and verify that pass before starting another mode. [`REVAMP.md`](REVAMP.md) provides a proven structural-additive pipeline (ground in the real component → 3–4 design directions → adversarial judge → self-contained interactive mockup with every honest state → pixel-critique loop → implementation spec → gated implementation) plus per-surface checklists for trace/provenance UIs, agent chat, proposal/diff review, status & latency feel, layout, and content quality. [`examples/trace-revamp/`](examples/trace-revamp/) is the full worked case: a production trace tab taken from flat text dump to a provenance rail with a tri-signature seal and three honest states — mockup and engineer-ready spec included.

When the needed change is subtraction rather than redesign, [`DECLUTTER.md`](DECLUTTER.md)
provides the companion mode: baseline the real surface, trace each visible promise to a
handler/data/network effect, freeze the protected trust contract, classify candidates as
PRESERVE/REPAIR/DEFER/MERGE/REMOVE, and verify the calmer result without deleting consent,
provenance, honest states, recovery, navigation, or unique capabilities. Its advisory
[`scripts/clutter-audit.mjs`](scripts/clutter-audit.mjs) reports rendered/semantic load,
repetition, exposed clusters, passive regions, clipping, and protected selectors; it never
pronounces a component useless or optimizes a composite clutter score.
...
instances), how the fix modes [`DECLUTTER.md`](DECLUTTER.md) / [`REVAMP.md`](REVAMP.md) /
[`PRETTIFY.md`](PRETTIFY.md) compose, and how the lifecycle modules — [`REDTEAM.md`](REDTEAM.md) (attack),
...
SKILL.md            the universal protocol: ground rules, journey archetypes A0–A6,
                    the Bar, 13 hard-won traps (U1–U13), finding format, improve loop
DECLUTTER.md        the subtractive structural mode: function ledger + protected contract,
                    PRESERVE/REPAIR/DEFER/MERGE/REMOVE rubric, before/after density and
                    reachability proof; fewer elements is never a standalone success metric
...
PRETTIFY.md         the presentation-only polish subsystem: B9 exploded into the
                    ...
                    B1–B11), and the inviolable "additive to trust, never a tradeoff"
                    guardrails (a11y-snapshot diff, masked honest-state pixel diff)
HANDOFF.md          the authorized handoff: a §6 finding + fix → review packet and, only
                    with publish authority, a PR via BetterPRHandoff (per-surface lanes ·
                    verified demo · live presence/absence proof · diagram · QA packet);
                    deployed independent proof is required before "shipped"
...
scripts/clutter-audit.mjs  conservative rendered-DOM inventory (semantic load, repeated
                           controls/text, exposed clusters, passive/empty regions, clipping,
                           protected selectors) — advisory; feeds the DECLUTTER ledger
scripts/live-signal.mjs    additive raw-response presence or hydrated readiness + true-node
                           removal proof; other dispositions use live pixels/a11y/journeys
...
**Codex / Cursor / Gemini CLI / aider / anything else:** clone it anywhere and point your agent at it — `AGENTS.md` at the repo root is the entry prompt (it routes to `SKILL.md`). The YAML frontmatter in SKILL.md is optional skill-discovery metadata for hosts that support it; the executable protocol remains plain markdown plus dependency-light Node scripts (Playwright resolved from any repo that has it).
...
→ choose exactly one primary DECLUTTER / REVAMP / PRETTIFY mode by mechanism → re-run → re-score
...
Extracted from real QA sessions on production agentic apps — every trap in U1–U13 cost real debugging time once (frozen CDP screenshot pipelines, per-action consent resets, charset mojibake, CSS class collisions hiding entire components, concurrent agent writers, SPA-shell false negatives, capability-label lies, and element-count cargo cults). Trace-UI design vocabulary informed by [Agent Prism](https://github.com/evilmartians/agent-prism) (Evil Martians, MIT) — adopted as a reference, not a dependency.
