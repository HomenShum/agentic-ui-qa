...
| QA run mode / mutation boundary | **READ-ONLY DIAGNOSTIC by default.** Opening a workspace, editing files, importing credentials, or invoking BYOK/Ollama requires explicit SANDBOX DOGFOOD authorization and a named disposable workspace. |
...
- **A0 Smoke (READ-ONLY):** launch Electron app (computer-use `open_application`) → Settings auto-opens; inspect the unconfigured state and built-in Hub if already available, then screenshot light/dark. Do not import provider config or open a writable workspace. Those setup steps require a named disposable workspace under SANDBOX DOGFOOD.
