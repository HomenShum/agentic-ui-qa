...
| Auth path | Viewing sample/shared decks: anonymous. CREATING decks: access code — capture without echo from repo root: `$previewCode = ((npx convex env get NODESLIDE_PREVIEW_ACCESS_CODE --prod) -join '').Trim()`; never print/paste it and clear the variable after the authorized run |
| QA run mode / mutation boundary | **READ-ONLY DIAGNOSTIC on production by default.** Viewing the sample is read-only; deck creation, live-model edits, consented egress, accept/publish, and persistence require a disposable local/test deck or explicit AUTHORIZED PRODUCTION scope. |
...
· J3 trace audit · J4 export/present · J5 themes/responsive · J6 adversarial). In READ-ONLY
DIAGNOSTIC run only J0 against the landing/sample in a disposable browser context; J1–J6
require the named sandbox or explicit authorization. Within that boundary, use the repo
journeys verbatim.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2 | 2 | 2 | 2 | 2 | 1 | 2 | 1 | 2 | 2 | — | 2026-07-13 | **Historical B1–B10 score: 18/20; B11 was not scored** (report: `parity-studio/artifacts/nodeslide-bar-opus-2026-07-13/`; recorded in `parity-studio/.qa/memory/`). B1–B8 driven headlessly with atomic artifacts; B9/B10 scored from evidence. B9=2: shell is designed-tooling-grade; one flat surface = trace detail (redesign in flight). B10=2: verdict-first, consent-transparent copy, `[source needed]` discipline; one blemish = F2 raw-error leak. Live GLM edit was degraded on prod: 3 samples = 1 graceful fallback + 2 raw Convex Server Error after 70–120s; 0 clean live edits. Honesty held; consent-off egress network-verified = 0. **Improvement targets: B6, B8; choose the fix mode from the proven mechanism.** 5 findings in the ledger. |
| 2 | 2 | 2 | 2 | 2 | 1 | 2 | 1 | — | — | — | 2026-07-12 | Baseline from live World Cup session + Trace redesign (B9–B11 predate this row). B6: create-path 30s timeout falls back. B8: first-run modal + collapsed consent disclosure cost cheap agents steps. |
